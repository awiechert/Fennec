...
/// ADCollisionParameters class object forward declaration
class ADCollisionParameters;

/// ADCollisionParameters class object inherits from Material object
...
class ADCollisionParameters : public Material
{
public:
    ...
    ADCollisionParameters(const InputParameters & parameters);
...
protected:
    ...
    Real trap_rule(Real f_xp1, Real f_x, Real dx);
    
    /// Wk Helpter function for van der Waals frequency
    Real Wk_integral(Real lb, Real eps, int l, int q);
    
    /// Wc Helpter function for van der Waals frequency
    Real Wc_integral(Real lb, Real eps, int l, int q);
...
private:
    ...
    ADMaterialProperty<std::vector<std::vector<Real> > > & _beta_Br;	///< MaterialProperty for the Brownian frequency (m^3/s/GGp) ==> size = num_var x num_var
    ADMaterialProperty<std::vector<std::vector<Real> > > & _beta_CE;	///< MaterialProperty for the Convective frequency (m^3/s/GGp) ==> size = num_var x num_var
    ADMaterialProperty<std::vector<std::vector<Real> > > & _beta_GC;	///< MaterialProperty for the Gravitational frequency (m^3/s/GGp) ==> size = num_var x num_var
    ADMaterialProperty<std::vector<std::vector<Real> > > & _beta_TI;	///< MaterialProperty for the Inertial frequency (m^3/s/GGp) ==> size = num_var x num_var
    ADMaterialProperty<std::vector<std::vector<Real> > > & _beta_TS;	///< MaterialProperty for the Shear frequency (m^3/s/GGp) ==> size = num_var x num_var
    ADMaterialProperty<std::vector<std::vector<Real> > > & _beta_VW;	///< MaterialProperty for the van der Waals frequency (m^3/s/GGp) ==> size = num_var x num_var
    ADMaterialProperty<std::vector<std::vector<Real> > > & _alpha_Br;	///< MaterialProperty for the Brownian, Convective, and van der Waals efficiency (-) ==> size = num_var x num_var
    ADMaterialProperty<std::vector<std::vector<Real> > > & _alpha_GC;	///< MaterialProperty for the Gravitational efficiency (-) ==> size = num_var x num_var
    ...
    ADMaterialProperty<int> & _time_record;
    ADMaterialProperty<bool> & _done;
};
