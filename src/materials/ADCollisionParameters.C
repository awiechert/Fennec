/*!
*  \file ADCollisionParameters.C
*	\brief AD Material properties file to compute parameters associated with the population balance model
*	\details This file creates an ADMaterialProperty to calculate and store parameter information associated
...
#include "ADCollisionParameters.h"
 ...
 */
registerMooseObject("fennecApp", ADCollisionParameters);

InputParameters ADCollisionParameters::validParams()
{
...
ADCollisionParameters::ADCollisionParameters(const InputParameters & parameters):Material(parameters),
...
_beta_Br(declareADProperty<std::vector<std::vector<Real> > >("beta_Br")),
_beta_CE(declareADProperty<std::vector<std::vector<Real> > >("beta_CE")),
_beta_GC(declareADProperty<std::vector<std::vector<Real> > >("beta_GC")),
_beta_TI(declareADProperty<std::vector<std::vector<Real> > >("beta_TI")),
_beta_TS(declareADProperty<std::vector<std::vector<Real> > >("beta_TS")),
_beta_VW(declareADProperty<std::vector<std::vector<Real> > >("beta_VW")),
_alpha_Br(declareADProperty<std::vector<std::vector<Real> > >("alpha_Br")),
_alpha_GC(declareADProperty<std::vector<std::vector<Real> > >("alpha_GC")),
_time_record(declareADProperty<int>("time_record")),
_done(declareADProperty<bool>("done"))
{
...
void ADCollisionParameters::initQpStatefulProperties()
{
...
//Compute properties
void ADCollisionParameters::computeQpProperties()
{
...
void ADCollisionParameters::calculate_n0()
{
    Real sum = 0.0;
    for (unsigned int lm = 0; lm<_N.size(); ++lm)
    {
...
void ADCollisionParameters::calculate_vp()
{
...
void ADCollisionParameters::calculate_y()
{
    for (unsigned int l = 0; l<_size_bins; l++)
    {
...
void ADCollisionParameters::calculate_omega()
{
    for (unsigned int l = 0; l<_size_bins; l++)
    {
...
void ADCollisionParameters::calculate_avg_charge()
{
    for (unsigned int l = 0; l<_size_bins; l++)
    {
...
void ADCollisionParameters::calculate_std_charge()
{
    for (unsigned int l = 0; l<_size_bins; l++)
    {
...
void ADCollisionParameters::calculate_charge_bounds()
{
    for (unsigned int l = 0; l<_size_bins; l++)
    {
...
Real ADCollisionParameters::charge_dist(Real charge, int l)
{
	Real value = 0.0;
...
Real ADCollisionParameters::alpha_int(Real charge_l, Real charge_q, int l, int q)
{
...
void ADCollisionParameters::calculate_diffusion()
{
    for (unsigned int lm = 0; lm<_N.size(); ++lm)
    {
        int l = (int)lm/_nuc_bins;
...
void ADCollisionParameters::calculate_dispersion()
{
...
Real ADCollisionParameters::del(int l)
{
...
void ADCollisionParameters::calculate_beta_Br()
{
    for (unsigned int lm = 0; lm<_N.size(); ++lm)
    {
        int l = (int)lm/_nuc_bins;
        ...
        for (unsigned int qr = 0; qr<_N.size(); ++qr)
        {
            ...
            ADReal top = 4.0*3.14159*(_rad[l]+_rad[q])*(_diffusion[_qp][lm]+_diffusion[_qp][qr]);
            ADReal bot = ( (_rad[l]+_rad[q]) / ((_rad[l]+_rad[q])+sqrt(dl*dl+dq*dq)) ) + ( (4.0*(_diffusion[_qp][lm]+_diffusion[_qp][qr])) / ((_rad[l]+_rad[q])*sqrt(_vp[l]*_vp[l]+_vp[q]*_vp[q])) );
            
            ADReal before =  _beta_Br[_qp][lm][qr];
...
Real ADCollisionParameters::Reynolds(int qr)
{
	int q = (int)qr/_nuc_bins;
...
void ADCollisionParameters::calculate_beta_CE()
{
    for (unsigned int lm = 0; lm<_N.size(); ++lm)
    {
    	int l = (int)lm/_nuc_bins;
...
Real ADCollisionParameters::RelVel(int lm, int qr)
{
...
void ADCollisionParameters::calculate_beta_GC()
{
    for (unsigned int lm = 0; lm<_N.size(); ++lm)
    {
    	int l = (int)lm/_nuc_bins;
...
void ADCollisionParameters::calculate_beta_TI()
{
    for (unsigned int lm = 0; lm<_N.size(); ++lm)
    {
        int l = (int)lm/_nuc_bins;
...
void ADCollisionParameters::calculate_beta_TS()
{
    for (unsigned int lm = 0; lm<_N.size(); ++lm)
    {
        int l = (int)lm/_nuc_bins;
...
Real ADCollisionParameters::f_rad(Real r, int l, int q)
{
	Real t1, t2, t3;
...
Real ADCollisionParameters::g_rad(Real r, int l, int q)
{
	Real t1, t2;
...
Real ADCollisionParameters::trap_rule(Real f_xp1, Real f_x, Real dx)
{
    return dx*(f_xp1+f_x)/2.0;
}

/// Wk Helpter function for van der Waals frequency
Real ADCollisionParameters::Wk_integral(Real lb, Real eps, int l, int q)
{
    Real value = 0.0;
...
Real ADCollisionParameters::Wc_integral(Real lb, Real eps, int l, int q)
{
    Real value = 0.0;
...
void ADCollisionParameters::calculate_beta_VW()
{
    for (unsigned int lm = 0; lm<_N.size(); ++lm)
    {
        int l = (int)lm/_nuc_bins;
        for (unsigned int qr = 0; qr<_N.size(); ++qr)
        {
            ...
            if (isnan(rat))
            	rat = Wc/(Wk+1.0E-6);
            ADReal top = 1.0 + ( (4*(_diffusion[_qp][lm]+_diffusion[_qp][qr]))/((_rad[l]+_rad[q])*sqrt(_vp[l]*_vp[l]+_vp[q]*_vp[q])) );
            top = top*Wc;
            ADReal bot = 1.0+(rat)*( (4*(_diffusion[_qp][lm]+_diffusion[_qp][qr]))/((_rad[l]+_rad[q])*sqrt(_vp[l]*_vp[l]+_vp[q]*_vp[q])) );
...
void ADCollisionParameters::calculate_alpha_Br()
{
    for (unsigned int lm = 0; lm<_N.size(); ++lm)
    {
        int l = (int)lm/_nuc_bins;
        for (unsigned int qr = 0; qr<_N.size(); ++qr)
        {
            ...
            ADReal before = _alpha_Br[_qp][lm][qr];
...
void ADCollisionParameters::calculate_alpha_GC()
{
    for (unsigned int lm = 0; lm<_N.size(); ++lm)
    {
    	int l = (int)lm/_nuc_bins;
