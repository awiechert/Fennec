...
#include "ADShearMultiFragLinearMPB.h"
...
registerMooseObject("fennecApp", ADShearMultiFragLinearMPB);

InputParameters ADShearMultiFragLinearMPB::validParams()
{
...
ADShearMultiFragLinearMPB::ADShearMultiFragLinearMPB(const InputParameters & parameters)
: ADKernel(parameters),
...
Real ADShearMultiFragLinearMPB::KroneckerDelta(int i, int j)
{
    if (i==j)
        return 1.0;
    else
...
void ADShearMultiFragLinearMPB::VolumeFill()
{
    Real pi = 3.14159;
    Real c = pi/6.0;
    for (int k=0; k<_M; k++)
...
void ADShearMultiFragLinearMPB::NumberFill()
{
    Real pi = 3.14159;
    Real c = pi/6.0;
...
void ADShearMultiFragLinearMPB::BreakupRateFill()
{
    Real pi = 3.14159;
...
void ADShearMultiFragLinearMPB::FragmentFill()
{
...
ADReal ADShearMultiFragLinearMPB::computeQpResidual()
{
    ADReal source = 0.0;
    ADReal rate = 0.0;
    int k = _this_var;
