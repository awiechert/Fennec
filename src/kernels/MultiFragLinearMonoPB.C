...
#include "MultiFragLinearMonoPB.h"
 ...
 */
registerMooseObject("fennecApp", MultiFragLinearMonoPB);

InputParameters MultiFragLinearMonoPB::validParams()
{
...
MultiFragLinearMonoPB::MultiFragLinearMonoPB(const InputParameters & parameters)
: Kernel(parameters),
...
Real MultiFragLinearMonoPB::KroneckerDelta(int i, int j)
{
    if (i==j)
        return 1.0;
    else
...
void MultiFragLinearMonoPB::VolumeFill()
{
    Real pi = 3.14159;
    Real c = pi/6.0;
    for (int k=0; k<_M; k++)
...
void MultiFragLinearMonoPB::FragmentFill()
{
...
Real MultiFragLinearMonoPB::computeQpResidual()
{
    Real source = 0.0;
    Real rate = 0.0;
    int k = _this_var;
...
Real MultiFragLinearMonoPB::computeQpOffDiagJacobian(unsigned int jvar)
{
    int k = _this_var;
